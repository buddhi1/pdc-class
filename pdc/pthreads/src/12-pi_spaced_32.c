...
#define MAX_SPACE  32//assess the secondary cache line size

#define PI 3.1415926535 // for comparison of accuracy

 void *compute_pi (void *);

 int total_hits, total_misses, hits[MAX_THREADS][MAX_SPACE],
 ...
 main() {
 	...
 	for (i=0; i< num_threads; i++) {
 		hits[i][0] = i;
 		pthread_create(&p_threads[i], &attr, compute_pi, (void *) &hits[i][0]);
 	}
 	for (i=0; i< num_threads; i++) {
 ...
 void *compute_pi (void *s) { 
 	...
 	*hit_pointer = 0;
 	...
 	for (i = 0; i < sample_points_per_thread; i++) {
 		...
 		//local_hits ++;
 		(*hit_pointer)++;//Plot spaced_1: observe the effect of false-sharing and deleted local update
 		seed *= i;
 	}
